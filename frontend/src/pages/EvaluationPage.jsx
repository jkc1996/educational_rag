...
import { METRIC_CATEGORIES, METRIC_LABELS, getMetricAverages } from "../utils/metricUtils";
...
import MetricsInsightDialog from "./MetricsInsightDialog";
...
export default function EvaluationPage() {
  ...
  const [multiModels, setMultiModels] = useState(["groq", "gemini", "ollama"]);

  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState(null);               // single: array of rows
  const [compareResults, setCompareResults] = useState(null); // compare: { groq:[...], gemini:[...], ollama:[...] }
  ...
  const handleMetricToggle = (metric) => {
    setShownMetrics(prev => {
      ...
      if (old.includes(metric)) {
        if (old.length === 1) return prev; // keep at least one metric visible
  ...
  return (
    <Box sx={{ width: "100%", minHeight: "100vh", background: "#f7fafc", p: 0, m: 0, overflow: "auto" }}>
      {/* TOP BAR */}
      <Box sx={{ width: "100%", maxWidth: 1380, mx: "auto", mt: 3, mb: 1, px: { xs: 1, sm: 2, md: 3 } }}>
        <Paper
          elevation={2}
          sx={{
            background: "#fff",
          ...
          !results ? (
            ...
            <>
              ...
              >
                ...
                <Menu anchorEl={anchorEl} open={!!anchorEl} onClose={handleColMenuClose} keepMounted>
              ...
              >
                <Box sx={{ width: "100%", overflowX: "auto", borderRadius: "inherit" }}>
                  <MetricTable
                    results={results}
              ...
              <Box sx={{ width: "100%", mb: 2 }}>
                <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
                  <ToggleButtonGroup
                    ...
                    size="small"
                  >
                    <ToggleButton value="per-question" sx={{ px: 3, fontWeight: 600 }}>
                      PER QUESTION
                    </ToggleButton>
                    <ToggleButton value="aggregated" sx={{ px: 3, fontWeight: 600 }}>
                      AGGREGATED
                    </ToggleButton>
                  </ToggleButtonGroup>
                </Box>
      ...
      {/* INSIGHTS DIALOG */}
      {chartOpen && (
        <MetricsInsightDialog
          open={chartOpen}
          onClose={() => setChartOpen(false)}
          mode={evalMode}
          metricLabels={METRIC_LABELS}
          singleData={evalMode === "single" ? results : null}
          rawCompare={evalMode === "compare" ? compareResults : null}
          selectedModels={evalMode === "compare" ? multiModels : [model]}
          onQuestionClick={(rowId) => {
            // optional: scroll/flash the row in the table
            console.log("Insight dialog clicked question ID:", rowId);
          }}
        />
      )}
